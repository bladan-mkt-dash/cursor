...
Q2 2026 preset (Apr 1–Jun 30). Fetches and metrics exclude the current calendar day
(partial-day) even if the sidebar end date includes today.

Run from the project directory (where google-ads.yaml and .env live):

    streamlit run google_ads_ghl_conversion_report_q2_2026.py
...
def _q2_2026_default_end(today: date) -> date:
    """Sidebar default end: through yesterday within Q2, or full quarter end after Q2."""
    yesterday = today - timedelta(days=1)
    if today.year == 2026 and today <= Q2_2026_END:
        if today >= Q2_2026_START:
            cap = min(Q2_2026_END, yesterday)
            if cap >= Q2_2026_START:
                return cap
            return Q2_2026_START
        return Q2_2026_END
    return Q2_2026_END
...
def main() -> None:
    ...
    try:
        st.set_page_config(
            page_title="Google Ads + GHL — Q2 2026",
            layout="wide",
    ...
    st.title("Google Ads + GoHighLevel — Q2 2026 conversion report")
    st.caption(
        ...
        "**Today’s date is excluded** from API ranges and from Google Ads daily rows."
    )

    today = date.today()
    default_start = Q2_2026_START
    default_end = _q2_2026_default_end(today)
    ...
    yesterday = today - timedelta(days=1)
    effective_until = until_d if until_d < today else yesterday
    if since_d > effective_until:
        st.error(
            "After excluding today, there is no valid end date on or after the start date. "
            "Choose an earlier start or an end date before today."
        )
        st.stop()
    ...
    if not ads_daily.empty:
        ads_daily = ads_daily.loc[ads_daily["date"].dt.normalize() < pd.Timestamp(today)].copy()
