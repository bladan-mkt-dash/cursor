"""Streamlit: ZM Primary Care — Meta campaign + GHL Facebook/Instagram (hear-about-us)."""
...
st.set_page_config(page_title="ZM PC — Meta + GHL Performance", layout="wide")
st.title("ZM PC — Meta + GHL Performance")
st.caption(
    "Meta: token vars per `meta_client.py`, `META_AD_ACCOUNT_ID`. "
    "GHL: `GHL_ACCESS_TOKEN`, `GHL_LOCATION_ID`, optional `GHL_HEAR_ABOUT_US_FIELD_ID`. "
    "Insights end date is capped at **yesterday** so daily Meta totals stay complete."
)
...
with st.sidebar:
    ...
    until = st.date_input("End date", value=today)

until_effective = min(until, last_complete)
if since > until_effective:
    st.error("Start date must be on or before the last complete day (yesterday).")
    st.stop()

if until > last_complete:
    st.info(
        f"End date capped at **{last_complete.isoformat()}** for Meta daily completeness "
        "(today excluded)."
    )
...
try:
    report = fetch_campaign_daily_insights(
        ...
        until=until_effective.isoformat(),
    )
...
st.caption(
    ...
    f"same capped date range as above. Field label used for lookup: {GHL_HEAR_ABOUT_FIELD_NAME!r}."
)

try:
    ghl = fetch_facebook_instagram_conversions(
        since.isoformat(),
        until_effective.isoformat(),
