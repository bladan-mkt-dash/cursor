...
Compares the current Sunday–Friday window to the prior Sunday–Friday across
bookings, signups, traffic, paid media, and hear-about attribution. Includes
an auto-generated interpretation of what is driving week-over-week changes.
...
the full Sun–Fri week is complete. On Sunday through Friday, the window runs
from that week's Sunday through today.

Custom end date:

    python work_week_in_review.py --end 2026-06-12
...
def sunday_friday_range(*, end: date | None = None) -> tuple[date, date]:
    """Inclusive range from the Sunday on or before ``end`` through ``end``."""
...
def prior_sunday_friday_period(start: date, end: date) -> tuple[date, date]:
    """Prior Sunday–Friday window, matching span when the current week is partial."""
    prior_start = start - timedelta(days=7)
    prior_end = prior_start + (end - start)
...
def build_report(snap: WorkWeekSnapshot) -> str:
    ...
    lines.extend(
        [
            ...
            "for the Sunday–Friday week ending yesterday._",
            "",
        ]
    )
    return "\n".join(lines)
